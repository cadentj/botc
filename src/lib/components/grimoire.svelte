<script lang="ts">
	...
	import { CHARACTERS_BY_TYPE, HELPER_TOKENS } from '$lib/botc-data/trouble-brewing.svelte';
	...
	let { lobbyCode, characterToPlayer = $bindable({}) }: { lobbyCode: string; characterToPlayer: Record<string, string> } =
		$props();
	...
	const availableHelperTokens = $derived.by(() => {
		...
		return HELPER_TOKENS.filter((ht) => selectedCharacterNames.includes(ht.forCharacter));
	});
